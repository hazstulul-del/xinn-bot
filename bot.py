...
OWNER_ID = int(os.getenv("OWNER_ID", "ID_TELEGRAM_KAMU"))
VERSION = "v4.1.0" # Versi terbaru dengan Banner Video
...
# ============================================================
# START COMMAND (DENGAN BANNER VIDEO)
# ============================================================
...
def start(message):
    ...
    # ⚡ VIDEO BANNER — MASUKKAN URL DARI TUAN
...
def download_tiktok(message):
    ...
    try:
        ...
        if data.get("video"):
            ...
            bot.edit_message_text("❌ Gagal download. Cek link TikTok.", message.chat.id, msg.message_id)
    except Exception as e:
        bot.edit_message_text(f"❌ Error: {str(e)[:50]}", message.chat.id, msg.message_id)

# ============================================================
# INSTAGRAM DOWNLOADER (REAL)
# ============================================================
...
# ============================================================
# SPOTIFY DOWNLOADER (SEARCH -> YOUTUBE -> DOWNLOAD)
# ============================================================
def download_spotify(message):
    ...
    try:
        ...
        if data.get("status") == 200:
            ...
            if yt_data.get("status") == 200 and yt_data.get("data"):
                ...
                if dl_data.get("status") == 200 and dl_data.get("data"):
                    ...
                    if audio_url:
                        ...
                        bot.edit_message_text("❌ Gagal ambil audio dari YouTube.", message.chat.id, msg.message_id)
                else:
            ...
            bot.edit_message_text("❌ Gagal ambil info Spotify. Cek link.", message.chat.id, msg.message_id)
    except Exception as e:
        bot.edit_message_text(f"❌ Error: {str(e)[:50]}", message.chat.id, msg.message_id)
